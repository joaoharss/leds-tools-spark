...
import { isLocalEntity } from "../../../../../../language/generated/ast.js";
...
function generateAttParameters(attribute, is_abstract) {
    return expandToString `
${toString(generateTypeAttribute(attribute) ?? 'NOTYPE')} ${attribute.name.toLowerCase()}`;
}
function generateAttSendParameters(attribute, is_abstract) {
    return expandToString `
${attribute.name.toLowerCase()}`;
}
function generateAttSendClassParameters(cls, attribute, is_abstract) {
    return expandToString `
${cls.name.toLowerCase()}.${capitalizeString(attribute.name)}`;
}
function generateSetAtt(attribute, is_abstract) {
    return expandToString `
...
function generateRelationsParameter(cls, relations) {
    let node = "";
    for (const rel of relations) {
...
function generateRelationParameterText(cls, { tgt, card, owner }) {
    switch (card) {
        case "OneToOne":
            if (owner) {
...
function generateRelationSendParameterText(cls, { tgt, card, owner }) {
    switch (card) {
        case "OneToOne":
            if (owner) {
...
function generateRelationSendClassText(cls, { tgt, card, owner }) {
    switch (card) {
        case "OneToOne":
            if (owner) {
...
function generateSetRelation(cls, { tgt, card, owner }) {
    switch (card) {
        case "OneToOne":
            if (owner) {
