...
function generateDbSet(entities) {
    return entities.map(cls => `public DbSet<${cls.name}> ${cls.name}s { get; set; }`).join('\n');
}
function generateKeyDeclarations(all_entities) {
    return all_entities
...
function generateDbContextUsing(modules) {
    let usings = "";
    for (const mod of modules) {
...
function generateRelationConfiguration(cls, { tgt, card, owner }) {
